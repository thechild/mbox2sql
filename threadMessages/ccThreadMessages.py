...
## Chris' updated version to take an iterable list of my django Message objects instead of an mbox
def ccThread(iterableMailbox):
  if kModuleVerbose:
  ...
  for msg in iterableMailbox:
    ...
    # iterate through all the headers looking for the important ones - could speed this up by prefetching these when creating the Message object
    for h in msg.headers.all():
      if h.field.lower() == "references":
        refsHeader = h.text
      elif h.field.lower() == "message-id":
        msgIDHeader = h.text
      elif h.field.lower() == "in-reply-to":
        irtHeader = h.text

    #refsHeader=msg.headers.get("references","")
